        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('grupos.update', $grupo->id) }}" method="POST" class="p-8 space-y-6">
                    ...
                    <select name="horario_id"
                        ...
                        @foreach($horarios as $h)
                            <option value="{{ $h->id }}" @selected(old('horario_id', $grupo->horario_id) == $h->id)>
                                {{ $h->materia->nombre }} ({{ $h->hora_inicio }}) - {{ $h->user->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="pt-6 flex gap-4">
                    ...
                    <button type="submit"
                            class="w-2/3 bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-bold transition shadow-lg hover:shadow-green-200">
                        Actualizar Cambios
                    </button>
                </div>
            </form>
