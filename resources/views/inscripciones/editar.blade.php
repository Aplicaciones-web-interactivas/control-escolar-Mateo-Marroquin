    ...
    <div class="max-w-2xl mx-auto px-4 py-6">
        <div class="mb-6">
            <a href="{{ route('inscripciones.index') }}"
               class="text-gray-500 hover:text-blue-600 text-sm font-bold flex items-center transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('inscripciones.update', $inscripcion->id) }}" method="POST" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <select name="user_id"
                        ...
                        @foreach($usuarios as $u)
                            <option value="{{ $u->id }}" @selected(old('user_id', $inscripcion->user_id) == $u->id)>
                                {{ $u->name }} ({{ $u->clave_institucional }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="pt-4 flex gap-4">
                    ...
                    <button type="submit"
                            class="w-2/3 bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold transition shadow-lg">
