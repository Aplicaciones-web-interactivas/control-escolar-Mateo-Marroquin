        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('calificaciones.update', $calificacion->id) }}" method="POST" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <select name="user_id"
                            class="w-full p-3.5 border border-gray-200 rounded-xl bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none">
                        @foreach($usuarios as $u)
                            <option
                                value="{{ $u->id }}" @selected(old('user_id', $calificacion->user_id) == $u->id)>
                                {{ $u->name }}
                            </option>
                        @endforeach
                    </select>
